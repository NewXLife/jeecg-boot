package org.jeecg.sql.blood.parse;
...
import org.jeecg.sql.blood.exception.SQLParseException;
import org.jeecg.sql.blood.exception.UnSupportedException;
import org.jeecg.sql.blood.model.Block;
import org.jeecg.sql.blood.model.ColLine;
import org.jeecg.sql.blood.model.QueryTree;
import org.jeecg.sql.blood.model.SQLResult;
import org.jeecg.sql.blood.utils.Check;
import org.jeecg.sql.blood.utils.MetaCache;
import org.jeecg.sql.blood.utils.NumberUtil;
import org.jeecg.sql.blood.utils.ParseUtil;
    ...
    private void parseCurrentNode(ASTNode ast){
        if (ast.getToken() != null) {
		    ...
		    case HiveParser.TOK_CREATETABLE: //outputtable
		    	...
		    	MetaCache.getInstance(this.template).init(tableOut); //初始化数据，供以后使用
		    	break;
            case HiveParser.TOK_TAB:// outputTable
                ...
                MetaCache.getInstance(this.template).init(tableOut2); //初始化数据，供以后使用
                break;
            case HiveParser.TOK_TABREF:// inputTable
                ASTNode tabTree = (ASTNode) ast.getChild(0);
                String tableInFull = fillDB((tabTree.getChildCount() == 1) ? BaseSemanticAnalyzer.getUnescapedName((ASTNode) tabTree.getChild(0))
                		: BaseSemanticAnalyzer.getUnescapedName((ASTNode) tabTree.getChild(0)) + SPLIT_DOT + BaseSemanticAnalyzer.getUnescapedName((ASTNode) tabTree.getChild(1))
                		);
                String tableIn = tableInFull.substring(tableInFull.indexOf(SPLIT_DOT) + 1);	
                inputTables.add(tableInFull);
                MetaCache.getInstance(this.template).init(tableInFull); //初始化数据，供以后使用
                queryMap.clear();
                String alia = null;
            	...
            	if (ast.getChild(0).getType() == HiveParser.TOK_ALLCOLREF) { 
            		...
            		if (!isSub) { //处理直接select * 的情况
                		...
                		for (String tables : tableArr) {
        					...
        					List<String> colByTab =  MetaCache.getInstance(this.template).getColumnByDBAndTable(tables);
...
//            case HiveParser.TOK_UNION:  //合并union字段信息
    ...
    public List<SQLResult> parse(String sqlAll) throws Exception{
    	return parse(sqlAll, null);
    }
     
    public List<SQLResult> parse(String sqlAll, HiveJdbcTemplate template) throws Exception{
	...
	private void putDBMap() {
    	for (String table : outputTables) {
    		List<String> list =  MetaCache.getInstance(this.template).getColumnByDBAndTable(table);
